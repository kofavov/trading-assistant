...
//        LocalDate stopDay = lastDayInList.plusDays(countDaysForPredict);
        ...
        getNewData(data, stopDay);
        data = data.subList(0, countDaysForPredict);
        Collections.reverse(data);
        return data;
    }

    private void getNewData(List<Case> data, LocalDate stopDay) {
        while (!data.get(0).getDate().equals(stopDay)) {
            ...
            double avg = data.subList(0, 7).stream().mapToDouble(Case::getValue).average().getAsDouble();
            setFuturePoint(data,avg);
        }
    }
}
